...
fn main() {
	...
	mut seeds := []u64{}
	seed_ranges := lines.first().split(': ').last().split(' ')

	for x := 0; x < seed_ranges.len; x+=2 {
		for y := seed_ranges[x].u64(); y < seed_ranges[x].u64() + seed_ranges[x+1].u64(); y+=1 {
			seeds << y
		}
	}
	...
	println(arrays.min(seeds.map(fn [maps] (seed u64) u64 {
		...
		return s
	}).filter(fn [seeds] (i u64) bool {return seeds.any(it == i)})) or {-1})
}
